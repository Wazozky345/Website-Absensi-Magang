...
if (!isset($milestone_list) || empty($milestone_list)) {
    $milestone_list = [
        '07' => ['judul' => 'Milestone 1 (Juli)', 'status' => 'Selesai', 'operasional' => '-', 'it' => '-'],
        '08' => ['judul' => 'Milestone 2 (Agustus)', 'status' => 'Berjalan', 'operasional' => '-', 'it' => '-'],
        '09' => ['judul' => 'Milestone 3 (September)', 'status' => 'Pending', 'operasional' => '-', 'it' => '-'],
        '10' => ['judul' => 'Milestone 4 (Oktober)', 'status' => 'Pending', 'operasional' => '-', 'it' => '-']
    ];
}
?>
<!DOCTYPE html>
...
<body class="flex h-screen overflow-hidden text-gray-800 bg-[#F4F7FE]">
                ...
                <div class="bg-white/95 backdrop-blur-md rounded-3xl p-6 shadow-sm border border-white lg:col-span-2">
                    ...
                    <div class="flex justify-between items-center mb-6">
                        <div class="flex items-center gap-4">
                            <h2 class="text-xl font-bold text-gray-800"><?= $nama_bulan_indo[$bulan_aktif] ?? 'Bulan Ini'; ?></h2>
                    ...
                    <div class="grid grid-cols-7 gap-2">
                        ...
                        <?php for($d = 1; $d <= $total_hari; $d++): 
                            ...
                            if (isset($agenda_list[$d]) && !empty($agenda_list[$d])): 
                                ...
                                elseif ($kategori_item === 'Bimbingan') { $warna = 'amber'; $icon = '👨‍🏫'; }
                                ...
                                if ($kategori_item === 'Bimbingan') {
                                    $onclick_action = "lihatDetailBimbingan('$js_judul', '{$item_agenda['tanggal']}', '$jam_tampil', '$js_deskripsi')";
                                } else {
                                    $onclick_action = "bukaModalCRUD('edit', '{$item_agenda['id']}', '$js_judul', '$kategori_item', '{$item_agenda['tanggal']}', '$jam_tampil', '$offset_tampil', '$js_deskripsi')";
                                }
                        ?>
                            ...
                            <div class="relative group">
                                ...
                                <div class="flex justify-between items-start gap-2">
                                    ...
                                    <button onclick="bukaModalMilestone('<?= $bln_key; ?>', '<?= $ms_judul; ?>', '<?= $ms['status']; ?>', '<?= $ms_oper; ?>', '<?= $ms_it; ?>')" 
                                            class="text-gray-400 hover:text-blue-600 p-2 rounded-xl hover:bg-gray-50 transition opacity-100 lg:opacity-0 lg:group-hover:opacity-100 focus:opacity-100">
            ...
            <form method="POST" action="" id="formAgenda">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <input type="text" name="judul_agenda" id="in-judul" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-blue-500 text-gray-700 font-medium">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="pengingat_offset" id="in-offset" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-500 text-gray-700 font-medium">
                                ...
                                <option value="20">20 Jam Sebelum</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi_agenda" id="in-deskripsi" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:border-blue-500 resize-none text-gray-700 font-medium"></textarea>
                    </div>
                </div>

                <div class="flex items-center justify-between gap-3 pt-4 border-t border-gray-50">
                    <button type="submit" name="hapus_agenda" id="btnDelete" class="hidden bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold py-2.5 px-4 rounded-xl transition text-sm" onclick="return confirm('Yakin ingin menghapus agenda ini?');">Hapus Agenda</button>
                    
                    <div class="flex items-center gap-2 ml-auto">
                        <button type="button" onclick="tutupModalCRUD()" class="border border-gray-200 hover:bg-gray-50 text-gray-500 font-bold py-2.5 px-4 rounded-xl transition text-sm">Batal</button>
                        <button type="submit" name="simpan_agenda" id="btnSubmit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg shadow-blue-100 transition text-sm">Simpan</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <div id="milestoneModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/50 p-4 backdrop-blur-sm transition-all">
        <div class="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl border border-gray-100 transform transition-all scale-95 opacity-0 duration-300" id="milestoneModalBox">
            ...
            <form method="POST" action="" id="formMilestone">
                ...
                <div class="space-y-4 mb-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 mb-1">Status Progres</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 mb-1">Target Operasional</label>
                        <textarea name="operasional_milestone" id="ms-operasional" rows="3" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:border-blue-500 text-gray-700 font-medium"></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-600 mb-1">Target IT</label>
                        <textarea name="it_milestone" id="ms-it" rows="3" required class="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm focus:outline-none focus:border-blue-500 text-gray-700 font-medium"></textarea>
                    </div>
                </div>

                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-50">
                    <button type="button" onclick="tutupModalMilestone()" class="border border-gray-200 hover:bg-gray-50 text-gray-500 font-bold py-2.5 px-4 rounded-xl transition text-sm">Batal</button>
                    <button type="submit" name="ubah_milestone" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-lg shadow-blue-100 transition text-sm">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function bukaModalMilestone(bulanKey, judul, status, operasional, it) {
            ...
            setTimeout(() => {
                document.getElementById('milestoneModalBox').classList.remove('scale-95', 'opacity-0');
                document.getElementById('milestoneModalBox').classList.add('scale-100', 'opacity-100');
            }, 50);
        }

        function tutupModalMilestone() {
            document.getElementById('milestoneModalBox').classList.remove('scale-100', 'opacity-100');
            document.getElementById('milestoneModalBox').classList.add('scale-95', 'opacity-0');
            setTimeout(() => { document.getElementById('milestoneModal').classList.add('hidden'); }, 200);
        }
        ...
        // Fungsi Popup khusus melihat detail Bimbingan Mentor (Read-Only)
        function lihatDetailBimbingan(judul, tanggal, waktu, deskripsi) {
            Swal.fire({
                title: judul,
                html: `
                    <div class="text-left mt-4 text-sm space-y-2">
                        ...
                        <div class="whitespace-pre-line mt-3 bg-amber-50 p-3 rounded-xl border border-amber-200 text-amber-800 leading-relaxed">
                            ${deskripsi}
                        </div>
                    </div>
                `,
        ...
        <div class="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl border border-amber-100 transform transition-all scale-100 opacity-100">
            ...
            <div class="text-center mb-6">
                ...
                <p class="text-xs text-amber-600 font-semibold bg-amber-50 inline-block px-3 py-1 rounded-full mb-3">
                    ...
                    <span class="block text-[10px] text-amber-500 font-normal mt-0.5">(Diingatkan <?= $agenda_besok['offset']; ?> jam sebelum acara)</span>
                </p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (!statusDitutup) document.getElementById('reminderModal').classList.remove('hidden');
        });
